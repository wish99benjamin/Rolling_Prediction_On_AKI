...
class LSAN(nn.Module):
    def __init__(self, dict_len, embedding_dim,  transformer_hidden, attn_heads, transformer_dropout, transformer_layers):
        ...
        self.attnweight = np.array([])
         
    def forward(self, batch_input, value, lstm=True):
        #print("new")
        # print(batch_input)
        input_embedding = self.embeds(batch_input)
        #print(input_embedding)
        #print(value)
        input_embedding =input_embedding + value.unsqueeze(3)
        #print(input_embedding)
        ...
        # Attention Analysis
        # diagnosis_code_mask = (batch_input != 51).type(torch.float32)
        ...
        # self.attnweight = code_attn_weight
        ...
        binary_output = torch.sigmoid(prediction_output)
